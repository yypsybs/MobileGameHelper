...
# help auto detect package name
package_name_keyword = ('com.android.documentsui',)
...
class Connection(ConnectionAttr):
    ...
    def list_game_packages(self, keywords=package_name_keyword, show_log=True):
        """
        Args:
            keywords:
            show_log:
        ...
        elif package_name_keyword:
            packages = [p for p in packages if any([k in p.lower() for k in package_name_keyword])]
        return packages

    def detect_package(self, keywords=package_name_keyword, set_config=True):
        """
        Show all possible packages with the given keyword on this device.
        """
        logger.hr('Detect package')
        packages = self.list_game_packages(keywords=keywords)
        ...
        if len(packages) == 1:
            ...
            logger.critical(
                f'Multiple {keywords} packages found, auto package detection cannot decide which to choose, '
